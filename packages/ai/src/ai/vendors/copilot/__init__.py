...
from ai.core import AuthResult, BaseVendor
...
def _build_dynamic_headers(context: Context) -> dict[str, str]:
    from ai.types import ImageContent, ToolResultMessage, UserMessage
...
class _TokenManager:
    ...
    def _save_token(self, github_token: str) -> None:
        from ai.paths import auth_file, data_dir
        f = auth_file()
...
async def _login_github(domain: str = "github.com") -> str:
    from ai.utils.auth import device_code_flow
...
class CopilotVendor(BaseVendor):
    ...
    async def sync_models(self, ttl: float = 86400, force: bool = False) -> tuple[Model, ...]:
        from ai.vendors.copilot._model_sync import sync_models

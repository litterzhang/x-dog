...
class CopilotProvider(BaseProvider):
    ...
    def _get_vendor(self):
        if self._vendor is None:
            from ai.vendors.copilot import CopilotVendor
    ...
    def _get_protocol(self, protocol_id: str):
        if protocol_id not in self._protocols:
            if protocol_id == "openai-completions":
                ...
                from ai.protocols.anthropic_messages import AnthropicMessagesProtocol
                self._protocols[protocol_id] = AnthropicMessagesProtocol()
            elif protocol_id == "openai-responses":
                from ai.protocols.openai_responses import OpenAIResponsesProtocol
    ...
    def _resolve(self, name: str) -> Model:
        ...
        if m is not None:
            return m
        from ai.vendors.copilot._model_sync import get_synced_model
    ...
    def models(self) -> tuple[Model, ...]:
        from ai.vendors.copilot._model_sync import list_models
        return list_models()

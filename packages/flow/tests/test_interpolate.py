"""Tests for flow.interpolate — dotted-path interpolation over typed wire values."""
...
def test_walk_into_scalar_is_empty() -> None:
    # {{name.field}} where name is a scalar -> empty, not an error.
...
def test_resolve_path_helper() -> None:
    v = {"a": {"b": [10, 20]}}
    assert resolve_path(v, ["a", "b", "1"]) == 20
    assert resolve_path(v, ["a", "x"]) is None
    assert resolve_path(v, ["a", "b", "9"]) is None
    assert resolve_path(42, ["a"]) is None

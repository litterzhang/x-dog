...
from claw.config import ClawConfig, get_config_path, get_state_dir, load_config
...
def start(config_path: str | None, foreground: bool) -> None:
    ...
    from claw.core.runtime.gateway import read_pid
    ...
    if foreground:
        ...
        from claw.core.runtime.gateway import run_gateway
        run_gateway(config)
    else:
        _daemonize(config)
...
def tui(config_path: str | None, group_id: str) -> None:
    ...
    config = _resolve_config(config_path)
    from claw.channels.tui.tui_client import run_tui

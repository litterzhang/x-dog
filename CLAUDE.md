...
- `cli.py` — `xdog-flow` CLI: `validate`, `run` (`--timeout` per-node), `generate`, `graph`, `build`
- `builder/` — visual workflow builder: `serialize.py` (workflow↔JSON round-trip), `model.py` + `actions.py` (headless, fully-tested edit core, re-validates on every edit), `app.py` (TUI shell — **generated by a flow workflow**, see `examples/builder_codegen.json`). `xdog-flow build <file>` opens it.
